...
Las redes neuronales alimentadas con datos fijos (estáticos) no logran superar el umbral de 0.05-0.07 que marca el modelo lineal (Ridge). Necesitamos fine tuning.
...
En este caso, el modelo mostró un aprendizaje muy rápido. Según los registros de entrenamiento, el mejor rendimiento se alcanzó en la época 2, donde las métricas de validación fueron incluso superiores. A partir de la tercera época comenzó a aparecer sobreajuste, por lo que el Early Stopping y la recuperación del mejor checkpoint resultaron determinantes. En conjunto, aunque un R² de 0.10 pueda parecer modesto y confirma que el dataset LIAR es difícil de predecir únicamente a partir del texto, los resultados demuestran que ajustar un Transformer es significativamente más efectivo que emplear redes neuronales clásicas o modelos lineales.
...
Como factor externo (fuente), analizamos el ratio de falsedad por partido político y observamos que ciertos grupos ideológicos presentan una tendencia significativamente mayor a emitir declaraciones de baja credibilidad, lo que confirma la existencia de un sesgo partidista en la desinformación.
Como factor interno (contenido), examinamos los patrones lingüísticos asociados a distintos niveles de veracidad. Se identificó que las afirmaciones más dudosas tienden a emplear vocabulario más emocional, estructuras simplificadas y referencias genéricas, mientras que las afirmaciones más fiables incorporan datos específicos, cifras o elementos verificables. Estos rasgos ayudan a explicar cómo el mensaje en sí mismo puede contribuir a reforzar dinámicas polarizantes.
Este estudio muestra que la desinformación no es solo un conjunto de hechos incorrectos, sino un fenómeno complejo vinculado tanto a la identidad política del emisor como a las características internas del discurso, y cuya interacción favorece mecanismos de polarización y manipulación emocional.
...
El dataset LIAR es relativamente corto y con afirmaciones breves.
...
La relación entre vocabulario y veracidad es muy 
directa.
...
Los modelos contextuales no se han afinado (no hubo fine-tuning).
